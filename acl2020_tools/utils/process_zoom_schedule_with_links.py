...
def main(schedule_csv: str, outbase: str) -> None:
    ...
    schedule_df.loc[:, 'session_name'] = schedule_df.loc[:, "uniqueid"].apply(lambda x: x.split('.')[-1])
    schedule_df.loc[:, "zoom_join_link"] = schedule_df.loc[:, "join_link(for Attendees)"]
